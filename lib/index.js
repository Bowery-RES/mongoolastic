...
const SyncType = require('./constants').SyncType;
const syncDependantIndexes = require('./addons/syncDependantIndices');
const DependantIndexOperation = require('./constants').DependantIndexOperation;
...
module.exports = function(schema, options, version = 5) {
  ...
  function esOptions() {
    ...
    if (!options.syncType) {
      options.syncType = SyncType.CURRENT_INDEX;
    }

    if (!Object.values(SyncType).includes(options.syncType)) {
      throw new Error('Invalid sync type');
    }

    options.syncDependantIndexes = [
      SyncType.DEPENDANT_INDEXES,
      SyncType.CURRENT_AND_DEPENDANT_INDEXES,
    ].includes(options.syncType);

    options.syncCurrentIndex = [
      SyncType.CURRENT_INDEX,
      SyncType.CURRENT_AND_DEPENDANT_INDEXES,
    ].includes(options.syncType);

    options.dependantIndexes = options.dependantIndexes || [];
  ...
  schema.statics.esDeleteIndex = deleteIndex;
...
module.exports.SyncType = SyncType;
  ...
  return utils.run(callback, (resolve, reject) => {
    ...
    esOptions.client.indices.exists(
      ...
      (err, exists) => {
        ...
        if (exists) {
          esOptions.client.indices.delete(
            { index: esOptions.index },
            (err, result) => {
              if (err) {
                reject(err);
    ...
    if (!esOptions.filter || esOptions.filter(doc)) {
      if (esOptions.syncDependantIndexes && !data.wasNew) {
        await syncDependantIndexes(
          doc,
          esOptions,
          DependantIndexOperation.UPDATE
        );
      }

      if (!esOptions.syncCurrentIndex) {
        return;
      }

      if (esOptions.populate) {
        await doc.populate(esOptions.populate).execPopulate();
      }

      try {
        let res = await doc.esIndex(
          data.wasNew ? false : { unset: data.unset }
        );
        if (esOptions.script && data.unset && data.unset.length) {
          res = await doc.esUnset(data.unset);
        }
 ...
 * @param {Object} doc
 */
async function postRemove(doc, next) {
  if (doc && doc.esOptions) {
    const esOptions = doc.esOptions();

    if (esOptions.syncDependantIndexes) {
      try {
        await syncDependantIndexes(
          doc,
          esOptions,
          DependantIndexOperation.REMOVE
        );
      } catch (err) {
        next(err);
      }
